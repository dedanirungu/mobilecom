...
import {createIgnoreTable, listRecord, saveRecord} from "../Helper"
...
function Ignore(props) { 
  ...
  const [ignore_list, setIgnoreList] = useState([]);
  ...
  const savePhone = async (name, phone) => {
     ...
     if (name) {
      if (phone) {
        ...
        if(editing){
          await saveRecord('ignore', record, edit_item.id );
        }else{
          await saveRecord('ignore', record);
        }
      ...
      <View style={styles.body}>
        ...
        <ScrollView>
          ...
          <View>
            { !loading ? 
            <View>         
              <Text style = {styles.viewTitle}>Ignore List</Text>

              {
              ignore_list.map((item, index) => (
                <TouchableOpacity
                    id={index} 
